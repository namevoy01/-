...
$result_p = mysqli_query($conn,$sqlp);
...
// ดึงข้อมูล ที่อยู่ การจอยตาราง
$sqladress = "SELECT ad.*,st.*, p.province_name
    FROM adress ad
    LEFT JOIN province p ON p.province_id = ad.Province
    LEFT JOIN student st ON st.st_ID = ad.st_ID
    WHERE ad.st_ID = '$st_ID'";
$resultad = mysqli_query($conn, $sqladress);
$ad = mysqli_fetch_object($resultad);
$chkprovince_id = $ad->Province;
...
<form method="post" action="profile_save.php" enctype="multipart/form-data">
    ...
    <select name="province_id" class="input" required>
      ...
      <?php while($p = mysqli_fetch_object($result_p)) { ?>
        <option value="<?php echo $p->province_id ?>" <?php echo ($p->province_id == $chkprovince_id) ? 'selected' : ''; ?>>
